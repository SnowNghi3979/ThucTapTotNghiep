    ...
    <div style={{display: 'flex', gap: '40px', justifyContent: 'center'}}>
      {Object.keys(data) && Object.keys(data)?.map((item) => {
        return (
          ...
          >
            <span style={{color: '#ea0a0a',fontSize: 30,}}>
              {item === 'users' && <UserOutlined />}
              {item === 'products' && <AppstoreOutlined />}
              {item === 'orders' && <ShoppingCartOutlined />}
              {item === 'brands' && <ShoppingCartOutlined />}
              {item === 'sliders' && <ShoppingCartOutlined />}
              {item === 'categories' && <ShoppingCartOutlined />}
              {item === 'posts' && <ShoppingCartOutlined />}
              {item === 'menus' && <ShoppingCartOutlined />}
              {item === 'configs' && <ShoppingCartOutlined />}
